...
class ResidentialTariff {
  ...
  static TariffCategory category = TariffCategory.lowTensionA;

  static const double demandCharge = 42.00;
  static final SlabRate lifeLine = SlabRate(start: 0, end: 50, price: 4.63);
  static final slabs = [
  ...
  static double unitFromEnergyCost(double cost) {
    final lifeLineMaxCost = lifeLine.end! * lifeLine.price;
    if (cost <= lifeLineMaxCost) {
      return cost / lifeLine.price;
    }
    ...
    for (final tariff in slabs) {
      double slabTo = tariff.end ?? double.infinity;
      double slabUnits = (slabTo - tariff.start + 1);
  ...
  // static double energyCost(double units) {
  //   double cost = 0;
